...
pub fn bbs_sig_and_accumulator() {
    fn run(use_setup_params: bool) {
        ...
        let pos_accumulated =
            positive_accumulator_get_accumulated(pos_accumulator.clone()).unwrap();
        let uni_accumulated =
            universal_accumulator_get_accumulated(uni_accumulator.clone()).unwrap();
        ...
        let (stmt_1, stmt_2, stmt_3, stmt_4, stmt_5, stmt_6, stmt_7, stmt_8, stmt_9) =
            if use_setup_params {
                setup_params.push(
                    &generate_setup_param_for_vb_accumulator_params(accum_params.clone()).unwrap(),
                );
                setup_params.push(
                    &generate_setup_param_for_vb_accumulator_public_key(accum_pk.clone()).unwrap(),
                );
                setup_params.push(
                    &generate_setup_param_for_vb_accumulator_mem_proving_key(mem_prk.clone())
                        .unwrap(),
                );
                setup_params.push(
                    &generate_setup_param_for_vb_accumulator_non_mem_proving_key(
